...
CREATE TABLE IF NOT EXISTS `Match1` (
  ...
  `venue` INT(9) NOT NULL,
  PRIMARY KEY (`idmatch`, `team1`, `team2`, `date1`,`venue`),
  ...
  INDEX `MATCH_VENUE_FK_idx` (`venue` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `Hosting` (
  ...
  CONSTRAINT `HOSTING_MATCH1_FK`
    FOREIGN KEY (`match1`)
    REFERENCES `Match1` (`idmatch`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `HOSTING_VENUE_FK`
    FOREIGN KEY (`venue`)
    REFERENCES `Venue` (`idvenue`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
...
CREATE TABLE IF NOT EXISTS `Competes` (
  ...
  CONSTRAINT `COMPETES_TEAM_FK`
    FOREIGN KEY (`team`)
    REFERENCES `Team` (`idteam`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `COMPETES_COMPETITION_FK`
    FOREIGN KEY (`competition`)
    REFERENCES `Competition` (`idcompetition`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
